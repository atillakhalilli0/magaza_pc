async function getComponents() {
   try {
      const res = await fetch("https://corepcdata.onrender.com/components");
      ...
      components.forEach((comp, index) => {
         ...
         // fiyat temizleme
         const prices = comp.price.replace(/\s+/g, "").match(/(\d+₼)/g);
         let discounted = "";
         let original = "";
         ...
         const productKey = `component-${index}`;
         const isFavorite = favorites.some(fav => fav.id === productKey);
         const isInCart = cart.some(item => item.id === productKey);

         card.innerHTML = `
          <!-- Favorite Button -->
          <button onclick="toggleFavorite('${productKey}', '${comp.name.replace(/'/g, "\\'")}', '${comp.mainImage}', '${discounted}', 'component', ${index})" 
          ...
          <a href="product-detail.html?type=component&id=${index}" class="block">
            <img src="${comp.mainImage}" alt="${comp.name}" class="w-full h-56 object-contain bg-gray-100 dark:bg-gray-700 p-4">
          </a>
          <div class="p-4">
            <h2 class="text-lg font-semibold mb-2 line-clamp-2 text-gray-900 dark:text-gray-100">${comp.name}</h2>
            ${priceHTML}
            <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1 mb-4">
              <li><b>P/N:</b> ${comp.specs["P/N"] || "-"}</li>
              <li><b>Qeyd:</b> ${comp.specs["Qeyd"] || "-"}</li>
            </ul>
            
            <!-- Action Buttons -->
            <div class="flex space-x-2">
              <button onclick="addToCart('${productKey}', '${comp.name.replace(/'/g, "\\'")}', '${comp.mainImage}', '${discounted}', 'component', ${index})"
              ...
              <a href="product-detail.html?type=component&id=${index}" 
      ...
      document.getElementById("components").innerHTML = "<p class='text-red-500 dark:text-red-400'>Veriler yüklenemedi.</p>";
   }
}
