async function getMonitors() {
   try {
      const res = await fetch("https://corepcdata.onrender.com/monitors");
      ...
      monitors.forEach((monitor, index) => {
         ...
         // fiyatları ayır
         const prices = monitor.price.replace(/\s+/g, "").match(/(\d+₼)/g);
         let discounted = "";
         let original = "";
         ...
         const productKey = `monitor-${index}`;
         const isFavorite = favorites.some(fav => fav.id === productKey);
         const isInCart = cart.some(item => item.id === productKey);

         card.innerHTML = `
          <!-- Favorite Button -->
          <button onclick="toggleFavorite('${productKey}', '${monitor.name.replace(/'/g, "\\'")}', '${monitor.mainImage}', '${discounted}', 'monitor', ${index})" 
          ...
          <a href="product-detail.html?type=monitor&id=${index}" class="block">
            <img src="${monitor.mainImage}" alt="${monitor.name}" class="w-full h-56 object-contain bg-gray-100 dark:bg-gray-700 p-4">
          </a>
          <div class="p-4">
            <h2 class="text-lg font-semibold mb-2 line-clamp-2 text-gray-900 dark:text-gray-100">${monitor.name}</h2>
            ${priceHTML}
            <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1 mb-4">
              <li><b>Ekran ölçüsü:</b> ${monitor.specs["Ekran ölçüsü"] || "-"}</li>
              <li><b>Görüntü imkanı:</b> ${monitor.specs["Görüntü imkanı"] || "-"}</li>
              <li><b>Tezlik:</b> ${monitor.specs["Tezlik"] || "-"}</li>
              <li><b>Panel növü:</b> ${monitor.specs["Panel növü"] || "-"}</li>
              <li><b>Zəmanət:</b> ${monitor.specs["Zəmanət"] || "-"}</li>
            </ul>
            
            <!-- Action Buttons -->
            <div class="flex space-x-2">
              <button onclick="addToCart('${productKey}', '${monitor.name.replace(/'/g, "\\'")}', '${monitor.mainImage}', '${discounted}', 'monitor', ${index})"
              ...
              <a href="product-detail.html?type=monitor&id=${index}" 
      ...
      document.getElementById("monitors").innerHTML = "<p class='text-red-500 dark:text-red-400'>Veriler yüklenemedi.</p>";
   }
}
