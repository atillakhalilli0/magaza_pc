async function getLaptops() {
   try {
      const res = await fetch("https://corepcdata.onrender.com/laptops");
      const laptops = await res.json();

      const container = document.getElementById("laptoplar");
      ...
      laptops.forEach((laptop, index) => {
         ...
         // fiyatları temizle
         const prices = laptop.price.replace(/\s+/g, "").match(/(\d+₼)/g);
         let discounted = "";
         let original = "";
         ...
         const productKey = `laptop-${index}`;
         const isFavorite = favorites.some(fav => fav.id === productKey);
         const isInCart = cart.some(item => item.id === productKey);

         card.innerHTML = `
          <!-- Favorite Button -->
          <button onclick="toggleFavorite('${productKey}', '${laptop.name.replace(/'/g, "\\'")}', '${laptop.mainImage}', '${discounted}', 'laptop', ${index})" 
          ...
          <a href="product-detail.html?type=laptop&id=${index}" class="block">
            <img src="${laptop.mainImage}" alt="${laptop.name}" class="w-full h-56 object-contain bg-gray-100 dark:bg-gray-700 p-4">
          </a>
          <div class="p-4">
            <h2 class="text-lg font-semibold mb-2 line-clamp-2 text-gray-900 dark:text-gray-100">${laptop.name}</h2>
            ${priceHTML}
            <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1 mb-4">
              <li><b>CPU:</b> ${laptop.specs["Prosessor (CPU)"] || "-"}</li>
              <li><b>GPU:</b> ${laptop.specs["Videokart (GFX)"] || "-"}</li>
              <li><b>RAM:</b> ${laptop.specs["RAM"] || "-"}</li>
              <li><b>SSD:</b> ${laptop.specs["SSD"] || "-"}</li>
            </ul>
            
            <!-- Action Buttons -->
            <div class="flex space-x-2">
              <button onclick="addToCart('${productKey}', '${laptop.name.replace(/'/g, "\\'")}', '${laptop.mainImage}', '${discounted}', 'laptop', ${index})"
              ...
              <a href="product-detail.html?type=laptop&id=${index}" 
      ...
      document.getElementById("laptoplar").innerHTML = "<p class='text-red-500 dark:text-red-400'>Veriler yüklenemedi.</p>";
   }
}
